...
exon = df['Region_Name'].str.contains('exon')
...
unique_exon = df[exon].groupby('Sample').Region_Name.nunique()

#sum bases per sample
bases_total = df.groupby(['Sample'])['Gap_Size'].sum()
bases_intron = df[intron].groupby(['Sample'])['Gap_Size'].sum()
bases_utr = df[utr].groupby(['Sample'])['Gap_Size'].sum()
bases_exon = df[exon].groupby(['Sample'])['Gap_Size'].sum()

###count samples per region
samples_per_region = df.groupby('Region_Name').Sample.nunique()

#format regions per sample for output
combine_region_count = pd.concat([unique_regions,unique_intron,unique_utr,unique_exon], axis=1)
combine_region_count.columns = 'Total','Intron','UTR','Exon'

#format bases per sample for output
combine_bases = pd.concat([bases_total,bases_intron,bases_utr,bases_exon], axis=1)
combine_bases.columns = 'Total','Intron','UTR','Exon'
