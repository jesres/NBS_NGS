...
v1.1- check for "chr" prefix in input file and remove (on read, file is not modified) if present
...
const string VERSION = "sanger_primer_find_v1.1";
...
bool ReadFileVar(string filename,vector<VARIANT>& our_vars)
{
	...
	while (getline(infile, line))
	{
		...
		if(first_line)
			...
			if(0 == dummy_var.chr.find("chr"))	//chr starts with "chr"
			{
				dummy_var.chr.erase(0, 3);	//chop the "chr"
			}
...
bool DefineConfig()
{
	...
	if(!infile.is_open()){	//open failed
		cerr << "Unable to read " << configname.c_str() << endl;
		return false;
	}
	string line;
	int line_num=1;
